...
#ifdef __PKVM_HYP__
static void dma_unuse_firstlevel(struct dma_pte *pte)
{
	do {
		if (dma_pte_present(pte))
		  ...
		  unsigned long last_pfn, struct iommu_pages_list *freelist)
{
	...
	if (domain->dma_fq) {
		/*
		 * Lazy mode: defer the flush. It is drained as a single
		 * domain-selective invalidation before the next page donation
		 * or when the host explicitly flushes the domain. We don't
		 * track the unmapped sub-range (same as host driver).
		 */
...
{
	/*
	 * If pKVM enabled, perform the flush only for lazy mode. For strict mode,
	 * pKVM does the flush after unmap and we need not request a flush again.
	 */
	if (!pkvm_enabled())
